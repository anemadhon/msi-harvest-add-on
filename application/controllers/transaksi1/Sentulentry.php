...
class Sentulentry extends CI_Controller {
    ...
    public function index(){
        $this->load->view('transaksi1/eksternal/sentulentry/list_view');
    }
    ...
    public function showListData(){
        ...
        $rs = $this->se_model->getSentulEntryHeader($date_from2, $date_to2, $status);
        $data = array();
    ...
    public function add(){
        ...
        $object['plant_user'] = $this->m_general->sap_plant_select_by_id($this->session->userdata['ADMIN']['plant']);
		...
		if($object['plants'] !== FALSE) {
			...
			foreach ($object['plants'] as $plantkey=>$plant) {
				...
				$object['plant_user'][$sGroup][$plant['OUTLET']] = $plant['OUTLET_NAME1']." (".$plant['OUTLET'].")";
			}

        }

        $this->load->view('transaksi1/eksternal/sentulentry/add_view', $object);
    }
    ...
    public function addData(){
        ...
        $sentulentry_detail['material_no'] = $this->input->post('detMatrialNo');
        $count = count($sentulentry_detail['material_no']);
    ...
    public function edit(){
        ...
        $this->load->view('transaksi1/eksternal/sentulentry/edit_view', $object);
    }
    ...
    public function addDataUpdate(){
        ...
        $sentulentry_details['material_no'] = $this->input->post('detMatrialNo');
        $count = count($sentulentry_details['material_no']);
    ...
    public function deleteData(){
        $id_sentulentry_header = $this->input->post('deleteArr');
        $deleteData = false;
        foreach($id_sentulentry_header as $id){
            if($this->se_model->sentulEntryDeleteHeader($id))
            $deleteData = true;
        }
        
        if($deleteData){
            return $this->session->set_flashdata('success', "Sentul Data Entry Berhasil dihapus");
        }else{
            return $this->session->set_flashdata('failed', "Sentul Data Entry Approved, Gagal dihapus");
        }
    }
    ...
    public function printpdf($id =""){
        ...
        if($id != ""){
            $data['data'] = $this->se_model->tampil($id);
